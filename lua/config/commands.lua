			...
			if vim.fn.has("win32") == 1 then
				...
				vim.system({
					...
					if obj.code == 0 then
						...
						vim.notify(
							"[" .. obj.code .. "] " .. "GitPush Failed! STDERR: " .. obj.stderr,
...
--- @param compile_options table
function AssemblyExplorerMSVC(is_remote, compile_options)
  ...
  -- TODO: Optimize batch file by env cache
	if is_remote then
    vim.notify("AssemblyExplorer: is remote")
...
--- @param is_remote bool
--- @param compile_options table
function AssemblyExplorerGCC(is_remote, compile_options)
	...
	if is_remote then
		scp_push(vim.api.nvim_buf_get_name(0), "/tmp/_temp_assembly_explorer_gcc.c", function(flag)
			if flag then
				local compile_command = custom_command
...
-- Example: AssemblyExplorer gcc -O3
vim.api.nvim_create_user_command("AssemblyExplorer", function(opts)
	if opts.args == nil then
		...
		local compile_options = opts.fargs or {}
